"""Long-context loss comparison across models (hebbian and baselines).
...
def load_model(path, device):
    ...
    if ckpt.get("model_class") == "HebbianConv":
        from models.hebbian_minimal import HebbianConv
    ...
    short = path.split("/")[-1].replace("model_", "").replace(".pt", "")
...
def main():
    ...
    # Infer model size from first model checkpoint
    ckpt = torch.load(args.models[0], map_location="cpu", weights_only=False)
    cfg = ckpt["config"]
    n_params = sum(p.numel() for p in HebbianMamba(cfg).parameters())
    if n_params >= 1e9:
        size_tag = f"{n_params/1e9:.0f}B"
    else:
        size_tag = f"{n_params/1e6:.0f}M"
    del ckpt
