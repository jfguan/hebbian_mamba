...
Surprisingly, we can remove the query and key projections in Gated Delta Net by directly using a shifted key:
...
We can also think of the token shift as copy/paste - after I see x, think of y — which does seem extremely limiting since associations are restricted to neighboring tokens.
...
Lastly, the shifted key model seems utilize it's keys "better" for storing information across its layers:
