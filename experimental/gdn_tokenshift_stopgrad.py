"""Experimental: GDN token shift with stop-gradient on keys.

Same as GDN token shift but keys are detached from the computation graph.
The memory loss cannot push gradients through keys into the backbone.
Tests whether the backbone adapts its hidden states for key quality.
"""
...
class GDNTSStopGradBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # token-shifted normalized keys (detached — no gradient through keys)
        rk = l2norm(x.detach().view(B, T, H, K)) / (K ** 0.5)
        wk = l2norm(F.pad(x.detach()[:, :-1], (0, 0, 1, 0)).view(B, T, H, K))
    ...
    def step(self, x, state=None):
        ...
        rk = l2norm(x.detach().view(B, H, K)) / (K ** 0.5)
...
class GDNTSStopGradLayer(nn.Module):
    def __init__(self, cfg: ModelConfig):
        super().__init__()
        self.delta_norm = RMSNorm(cfg.d_model)
        self.delta = GDNTSStopGradBlock(cfg.d_model, num_heads=cfg.num_heads, d_conv=cfg.d_conv, chunk_size=cfg.chunk_size)
...
class GDNTSStopGrad(nn.Module):
    def __init__(self, cfg: ModelConfig):
        super().__init__()
        self.cfg = cfg
        self.embedding = nn.Embedding(cfg.vocab_size, cfg.d_model)
        self.layers = nn.ModuleList([GDNTSStopGradLayer(cfg) for _ in range(cfg.n_layers)])
