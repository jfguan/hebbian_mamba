"""Experimental: GDN with SiLU removed from Q/K paths.

Same as standard GDN but Q and K skip the SiLU activation.
Tests whether SiLU key compression is responsible for worse
key quality compared to token shift.
"""
...
class GDNNoSiLUBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        q = l2norm(self.q_conv(self.q_proj(x)).view(B, T, H, K)) / (K ** 0.5)
        k = l2norm(self.k_conv(self.k_proj(x)).view(B, T, H, K))
    ...
    def step(self, x, state=None):
        ...
        q = l2norm(q_raw.view(B, H, K)) / (K ** 0.5)
        k = l2norm(k_raw.view(B, H, K))
...
class GDNNoSiLULayer(nn.Module):
    def __init__(self, cfg: ModelConfig):
        super().__init__()
        self.delta_norm = RMSNorm(cfg.d_model)
        self.delta = GDNNoSiLUBlock(cfg.d_model, num_heads=cfg.num_heads, d_conv=cfg.d_conv, chunk_size=cfg.chunk_size)
...
class GDNNoSiLU(nn.Module):
    def __init__(self, cfg: ModelConfig):
        super().__init__()
        self.cfg = cfg
        self.embedding = nn.Embedding(cfg.vocab_size, cfg.d_model)
        self.layers = nn.ModuleList([GDNNoSiLULayer(cfg) for _ in range(cfg.n_layers)])
