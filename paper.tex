...
\begin{itemize}
    ...
    \item Strong gains on code at 18M parameters: $+$0.94 nats over a parameter-matched baseline on Python after 6M tokens (surpassing its final validation loss after only ${\sim}$800K tokens); $+$0.05--0.11 nats on prose (PG-19). The 18M memory model outperforms the 100M parameter-matched baseline throughout its training run.
