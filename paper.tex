...
The memory model outperforms both baselines. Val loss is estimated over 10 random batches per checkpoint and is noisy; the apparent widening trend in Table~\ref{tab:prose_dynamics} should be interpreted cautiously. All models are still improving at 6M tokens. The dense BMMs in the memory mechanism train faster per step than Mamba's parallel scan at this scale, so the gap is larger at iso-FLOPs.
