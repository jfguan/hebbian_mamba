"""Hybrid model: SWA + Delta.

SWA layers: attention → conv + MLP.
Delta layers: delta memory → conv + MLP.
First layer gets an extra conv + MLP before attention.
"""
...
from models.components import CausalConv, GatedMLP, SlidingWindowAttention, DeltaBlock
...
class SWALayer(nn.Module):
    """Attention → conv + MLP. Optionally prepend extra conv + MLP."""
    ...
    def __init__(self, cfg: ModelConfig, extra_block: bool = False):
        ...
        self.attn = SlidingWindowAttention(
            cfg.d_model, num_heads=cfg.num_heads, window_size=cfg.swa_window,
        )
    ...
    def step(self, x, state=None):
        ...
        attn_out, attn_st = self.attn.step(self.norm_attn(x), attn_st)
        x = x + attn_out

        normed = self.norm_mlp(x)
        val, conv_st = self.conv.step(self.mlp.project_up(normed), conv_st)
        return x + self.mlp(normed, val), {"conv": conv_st, "attn": attn_st}


class DeltaLayer(nn.Module):
    """Delta memory → conv + MLP."""
...
class Hybrid(nn.Module):
    def __init__(self, cfg: ModelConfig):
        ...
        for i in range(cfg.n_layers):
            if i in delta_set:
                self.layers.append(DeltaLayer(cfg))
            else:
                self.layers.append(SWALayer(cfg, extra_block=(i == 0)))
